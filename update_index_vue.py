...
with open("scanner-app/resources/js/Pages/Scanner/Index.vue", "r", encoding="utf-8") as f:
    content = f.read()

# 1. Add new JS logic to script setup
new_js = """
...
"""
# Insert before the API Key CRUD section
content = content.replace("// API Key CRUD", new_js + "\n// API Key CRUD")

# 2. Replace the template block
new_template = """
<template>
    ...
    <div class="h-screen flex bg-[#131314] text-gray-100 font-sans overflow-hidden">
        ...
        >
            ...
            <div class="p-3">
                <button @click="newChat" class="w-full flex items-center gap-3 px-4 py-3 bg-[#2a2a2c] hover:bg-[#353538] text-sm font-medium rounded-xl transition border border-gray-700">
...
# Replace <template>...</template> and <style>...</style>
start_template = content.find("<template>")
end_template = content.rfind("</style>") + 8

content = content[:start_template] + new_template

with open("scanner-app/resources/js/Pages/Scanner/Index.vue", "w", encoding="utf-8") as f:
    f.write(content)
